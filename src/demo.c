...
void *fetch_for_detection(void *ptr)
{
    //in = get_image_from_stream(cap);
	in = get_image_from_stream_resize(cap, net.w, net.h, &in_img);
    if(!in.data){
...
void *detect_for_detection(void *ptr)
{
    float nms = .4;
    ...
    image croped_im = crop_image_with_box(det, l.w*l.h*l.n, demo_thresh, boxes, probs, demo_names, demo_alphabet, demo_classes);

	return 0;
}


void detection(char *cfgfile, char *weightfile, float thresh, const char *filename, char **names, int classes, 
	int frame_skip, char *prefix, char *out_filename, int http_stream_port, int dont_show)
{
    //skip = frame_skip;
    image **alphabet = load_alphabet();
    ...
    fetch_for_detection(0);
    detect_for_detection(0);
    disp = det;
	det_img = in_img;
    det = in;
    det_s = in_s;
...
void detection(char *cfgfile, char *weightfile, float thresh, const char *filename, char **names, int classes, int frame_skip, char *prefix, char *out_filename, int http_stream_port, int dont_show)
{
    fprintf(stderr, "Demo needs OpenCV for webcam images.\n");
}
#endif
...
image crop_image_with_box(image im, int num, float thresh, box *boxes, float **probs, char **names, image **alphabet, int classes){
    ...
    for(i = 0; i < num; ++i){
        ...
        if(prob > thresh){
            ...
            return crop_image(im, left, top, abs(right - left), abs(bot - top));
        }
    }
}
